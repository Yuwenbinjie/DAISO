                                ...
                                <div v-if="activeTabName=='Streams'">
                                    <div>
                                        ...
                                        <p class="bold fs18 my12">
                                            ...
                                            <span>{{ infoCal.startTime|filterDate }}</span>
                                        </p>
                                    </div>
                                    <div>
                                        <p class="bold mt20">
                                        ...
                                        <p class="bold fs18 my12">
                                            <i class="el-icon-watch" />
                                            <span>{{ infoCal.stopTime|filterDate }}</span>
                                        </p>
                                    </div>
                                </div>
                                <div v-else>
    ...
    methods: {
        ...
        async getInfo(){//请求流详情数据
            if (this.activeTabName == 'Streams'){
                ...
                this.sellTokenInstance = await new web3.eth.Contract(contract_abi, this.infoCal.projectSellTokenAddress);
                this.fundTokenInstance = await new web3.eth.Contract(contract_abi, this.infoCal.projectFundTokenAddress);
                this.sellTokenAddress = await this.sellTokenInstance.methods.symbol().call() //获取币种符号
                this.fundTokenAddress = await this.fundTokenInstance.methods.symbol().call()
            } else if (this.activeTabName == 'Projects' || this.activeTabName == 'MyProjects') {
                ...
                this.sellTokenInstance = await new web3.eth.Contract(contract_abi, this.info.projectSellTokenAddress);
                this.fundTokenInstance = await new web3.eth.Contract(contract_abi, this.info.projectFundTokenAddress);
                this.sellTokenAddress = await this.sellTokenInstance.methods.symbol().call()
                this.fundTokenAddress = await this.fundTokenInstance.methods.symbol().call()
                console.log()
            }
        },
        async getBalance(){//获取发送/接收者余额
            ...
            if (this.activeTabName == 'Streams'){
                let withdrawAmount
                if (now < this.infoCal.startTime){
                    delta = 0
                } else if (now < this.infoCal.stopTime){
                    delta = now - this.infoCal.startTime
                } else {
                    delta = this.infoCal.stopTime - this.infoCal.startTime
                }
                ...
                let duration = await this.info.stopTime - this.infoCal.exitStartTime
                let remainOfFundStream = await new bignumber(this.info.projectActualFundDeposit).minus(this.infoCal.exitProjectFundBalance)
                let ratePerSecondOfProjectFund = await new bignumber(remainOfFundStream).dividedBy(duration)
                let ratePerSecondOfProjectSell = await new bignumber(this.infoCal.exitProjectSellBalance).dividedBy(duration)

                fundBalance = await new bignumber(ratePerSecondOfProjectFund).multipliedBy(delta)
                fundBalance = await new bignumber(fundBalance).plus(this.infoCal.exitProjectFundBalance)

                if (now >= this.info.stopTime) {
                    let remainderOfProjectFund = new bignumber(remainOfFundStream).mod(duration)
                    fundBalance = new bignumber(fundBalance).plus(remainderOfProjectFund)
                }
                ...
                sellBalance = await new bignumber(ratePerSecondOfProjectSell).multipliedBy(delta)
                sellBalance = await new bignumber(this.infoCal.exitProjectSellBalance).minus(sellBalance)

                if (now >= this.info.stopTime) {
                    let remainderOfProjectFund = new bignumber(this.infoCal.exitProjectSellBalance).mod(duration)
                    fundBalance = new bignumber(fundBalance).plus(remainderOfProjectFund)
                }
        ...
        invest() {//增加最大金额
            ...
            if (now < this.info.startTime){
                this.$prompt('Please input invest amount：', this.$t('streamInfo.function.operation'), {
                    ...
                    if (remainder == 0){
                        await this.fundTokenInstance.methods.approve(this.allInstance.DAISOInstance.options.address, setMoneyWeb3(value))
                            .send({gas: 500000, from: this.user})
                        await this.fundTokenInstance.methods.mint(this.user, setMoneyWeb3(value))
                            .send({gas: 500000, from: this.user})
            ...
            if (this.activeTabName == 'Streams'){
                ...
                if(proposal.status == 0){
                    this.$prompt(this.$t('streamInfo.function.withdraw'), this.$t('streamInfo.function.operation'), {
                        ...
                        if (res) {
                            this.$alert("Release Success")
                            await this.getInfo()//重新获取流信息
                        } else {
                            this.$alert("Release Failure")
                        }
                    })
