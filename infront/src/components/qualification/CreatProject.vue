        ...
        <div class="px20">
            ...
            >
                ...
                <el-form-item :label="$t('createstream.create.fundTokenAddress')">
                    <el-input v-model="params.fundTokenAddress" />
                </el-form-item>
<!--                <el-form-item :label="$t('createstream.create.fundTokenAddress')">-->
<!--                    &lt;!&ndash; <el-autocomplete style="width:100%" valueKey="label" :fetch-suggestions="querySearch" v-model="params.tokenAddress" /> &ndash;&gt;-->
<!--                    <el-select-->
<!--                        style="width:100%"-->
...
export default {
    ...
    methods: {
        ...
        async save() {
            ...
            let res
            let sellTokenInstance = await new web3.eth.Contract(contract_abi, this.params.sellTokenAddress);
            //批准、铸币
            // await sellTokenInstance.methods.mint(this.user, "100000000000000000000000").send({
            //     gas: 500000,
            //     from: this.user
            // });
            await sellTokenInstance.methods.approve(this.allInstance.DAISOInstance.options.address, setMoneyWeb3(this.params.fundDeposit)).send({
                gas: 500000,
                from: this.user
            });
        ...
        async validate(params){
            try {
                ...
                if (setMoneyWeb3(params.sellDeposit) % (moment(this.dateTime[1]).unix() - moment(this.dateTime[0]).unix()) != 0){
                    throw new Error(this.$t('createstream.validate.sellDepositRemainder'))
                }
                if (setMoneyWeb3(params.fundDeposit) % (moment(this.dateTime[1]).unix() - moment(this.dateTime[0]).unix()) != 0){
                    throw new Error(this.$t('createstream.validate.fundDepositRemainder'))
                }
            }
            catch (err){
