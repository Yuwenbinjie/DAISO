...
import {contract_abi} from './erc_20_abi.js';
...
export async function getMoneyWithUnit(val, tokenAddress) {//获取带单位的金额
    let money = getMoney(val)
    return `${money} ${tokenAddress}`
}

export async function getPriceWithUint(val, tokenAddress) {//获取带单位的金额
    return `${val} ${tokenAddress}`
}