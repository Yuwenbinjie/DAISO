...
contract IArbitrables is IArbitrable, IEvidence,OwnableWithoutRenounce, PausableWithoutRenounce, Exponential{
    ...
    constructor(DAISO _daiso) public {
        OwnableWithoutRenounce.initialize(msg.sender);
        PausableWithoutRenounce.initialize(msg.sender);
        arbitratorAddress = address(0x840FBc33600572FeBFe42FE112dde67a80a5D109);
        daiso = _daiso;
...
import "./centralized/Arbitrator.sol";
import "./centralized/CappedMath.sol";
...
contract AutoAppealableArbitrator is Arbitrator{
    ...
    struct Dispute {
        IArbitrables arbitrated;  // The contract requiring arbitration.
    ...
    function createDispute(uint _choices, bytes memory _extraData) public payable returns(uint disputeID) {
        ...
        disputeID = disputes.push(Dispute({
            arbitrated: IArbitrables(msg.sender),
            choices: _choices,
            fees: msg.value,
            ruling: 0,
        ...
        emit DisputeCreation(disputeID, IArbitrables(msg.sender));
    }
    ...
    function giveRuling(uint _disputeID, uint _ruling) external onlyOwner {
        ...
        msg.sender.transfer(dispute.fees); // Avoid blocking.
    ...
    function appeal(uint _disputeID, bytes memory _extraData) public requireAppealFee(_disputeID, _extraData) payable {
        ...
        emit AppealDecision(_disputeID, IArbitrables(msg.sender));
    }
